...
public class MapPractice2 {
    ...
    public static void main(String[] args) {
        ...
        //display the names of the employees who are hired this year (2022)

        for (Map<String, Object> eachMap : listOfMaps) {
            for (Map.Entry<String, Object> eachEntry : eachMap.entrySet()) {
                if (eachEntry.getKey().equals("hired_date")) {
                    if (((LocalDate) eachEntry.getValue()).getYear() == 2022) {  //return type is object we need down-casting to cast it to LocalDate
                        System.out.println(eachMap.get("name") + " : " + eachEntry.getValue());
                    }
                }
            }
        }
        ...
        listOfMaps.get(listOfMaps.size()-1).replace("salary",(Integer)(listOfMaps.get(listOfMaps.size()-1)).get("salary")+50000);// we casted Object to Integer, we increased original salary by 50000
        System.out.println(listOfMaps);
